    ...
    This.CreateEditView = Backbone.View.extend({
        ...
        save: function () {
            ...
            if (!_.isEmpty(errors)) {
                ...
                this.model.checkGroupKey(this.$name.val());
                this.model.save(formData, {validate: false});
                
                app.mediator.publish('Groups: saved', this.model);
                store.groups.add(this.model);
                this.createFlashMessage();
                if (formData.stage === 'planned' || formData.stage === 'boarding' || formData.stage === 'before-start') {
                    app.mediator.publish('GroupList: group created', 'planned');
                    $('#futureGroups').prop("checked", true);
                }

                if (formData.stage === 'in-process' || formData.stage === 'offering') {
                    app.mediator.publish('GroupList: group created', 'in-process');
                    $('#currentGroups').prop("checked", true);
                }
                if (formData.stage === 'finished') {
                    app.mediator.publish('GroupList: group created', 'finished');
                    $('#endedGroups').prop("checked", true);
                }
                this.destroy();
            }
        },
        ...
        setStage: function (formData) {
            ...
            if (moment(currentDate).isBefore(moment(start).subtract(1, 'months'))) {
